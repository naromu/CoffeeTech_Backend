...
# Función auxiliar para verificar el estado "Activo" del usuario
def verificar_estado_activo(user: User, db: Session) -> bool:
    # Obtener el StatusType "user_role_farm"
    status_type = db.query(StatusType).filter(StatusType.name == "user_role_farm").first()
    if not status_type:
        logger.error("No se encontró el tipo de estado 'user_role_farm'")
        raise HTTPException(status_code=500, detail="Configuración de estado inválida")
    
    # Obtener el estado "Activo" para el tipo "user_role_farm"
    ...
    # Verificar que el usuario tenga el estado activo
    if user.status_id != active_status.status_id:
        logger.warning("El usuario no tiene el estado 'Activo' requerido")
        raise HTTPException(status_code=403, detail="Usuario no activo o no autorizado")
...
def create_farm(request: CreateFarmRequest, session_token: str, db: Session = Depends(get_db_session)):
    ...
    # **No se realiza la verificación del estado "Activo" aquí**
...
def list_farm(session_token: str, db: Session = Depends(get_db_session)):
    ...
    # **Verificación del estado "Activo" del tipo "user_role_farm"**
    ...
    try:
        # Realizar la consulta con el filtro adicional de estado activo
        farms = db.query(Farm, UnitOfMeasure, Status, Role).select_from(UserRoleFarm).join(
            ...
            UserRoleFarm.user_id == user.user_id,
            UserRoleFarm.status_id == active_status.status_id  # Filtrar por estado activo
        ).all()
...
def update_farm(request: UpdateFarmRequest, session_token: str, db: Session = Depends(get_db_session)):
    ...
    # **Verificación del estado "Activo" del tipo "user_role_farm"**
    try:
        verificar_estado_activo(user, db)
